input_str = """plaid fuchsia bags contain 5 light violet bags, 1 light yellow bag.
...
for row in input_list:
    row = row.strip(".")
    halves = row.split(" contain ")
    main_key = halves[0].rstrip("s")
    bags = halves[1].split(", ")
    main_value = dict()
    for bag in bags:
        if bag[0].isdigit():
    ...
    rules[main_key] = main_value


def root_bags(input_bag):
    parents = []

    for key in rules.keys():
        if input_bag in rules[key].keys():
            parents.append(key)

    for parent in parents:
        parents.extend(root_bags(parent))

    return parents


def leaf_bags(input_bag):
    children_count = 0

    if input_bag != "no other bags":
        for key in rules[input_bag].keys():
            value = rules[input_bag][key]
            children_count += value
            children_count += leaf_bags(key) * value
            
    return children_count


def solve1():
    found = set(root_bags("shiny gold bag"))
    return len(found)


def solve2():
    return leaf_bags("shiny gold bag")
