...
def solve1():
    ...
    for passport_str in input_list:
        fields = dict([tuple(field.split(":")) for field in passport_str.split()])
        if len(req_fields - set(fields.keys())) == 0:
            count += 1
    return count


def validate_height(height_str):
    height_int = int("".join(filter(str.isdigit, height_str)))
    return "cm" in height_str and 150 <= height_int <= 193 \
           or "in" in height_str and 59 <= height_int <= 76
...
def solve2():
    ...
    validator = {
        ...
        "hcl": lambda x: len(x) == 7 and x[0] == "#"
                         and all(map(lambda c: c.isdigit or ord("a") <= ord(c) <= ord("f"), x[1:])),
    ...
    for passport_str in input_list:
        fields = dict([tuple(field.split(":")) for field in passport_str.split()])
        if len(req_fields - set(fields.keys())) == 0:
            if all(map(lambda key: validator[key](fields[key]), fields.keys())):
                count += 1
