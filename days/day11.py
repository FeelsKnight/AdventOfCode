...
input_list = list(map(lambda x: c for c in x, input_str.split("\n")))
...
def get_neighbors(i, j):
    ...
    for row in last_state[i_lower:i_upper]:
        for col in row[j_lower:j_upper]:
            neighbors.append([last_state.index(row), row.find(col)])
    return neighbors
    ...
    for iteration in range(50):
        ...
        for row in last_state:
            for col in row:
                ...
                if last_state[i][j] == "L" and len(occupied_neighbors) == 0:
                    next_state[i][j] = "#"
                elif last_state[i][j] == "#" and len(occupied_neighbors) > 3:
                    next_state[i][j] = "L"
                else:
