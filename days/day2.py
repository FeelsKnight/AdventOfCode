input_str = """5-10 b: bhbjlkbbbbbbb
3-4 j: hjvj
8-9 p: pmljtsttp
...
def solve1():
    ...
    for row in input_list:
        row_halves = row.split(":")
        password = row_halves[1].strip()
        parameter_halves = row_halves[0].split(" ")
        symbol = parameter_halves[1]
        limits = list(map(lambda x: int(x), parameter_halves[0].split("-")))
        if limits[0] <= password.count(symbol) <= limits[1]:
            count += 1
    return count
...
def solve2():
    ...
    for row in input_list:
        ...
        parameter_halves = row_halves[0].split(" ")
        symbol = parameter_halves[1]
        positions = list(map(lambda x: int(x) - 1, parameter_halves[0].split("-")))
        is_in_first = len(password) > positions[0] and password[positions[0]] == symbol
        is_in_second = len(password) > positions[1] and password[positions[1]] == symbol
        if (not is_in_first and is_in_second) or (not is_in_second and is_in_first):
            count += 1
    return count
